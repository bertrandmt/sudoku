...
bool Analyzer::test_swordfish(const Value &value, const CandidateSet &cset1, const CandidateSet &cset2, const CandidateSet &cset3,
                                                  const EliminationSet &eset1, const EliminationSet &eset2, const EliminationSet &eset3) {
    // Verify each candidate set has 2-3 candidates for the value
    auto candidates1 = candidates(cset1, value);
    if (candidates1.size() < 2 || candidates1.size() > 3) return false;

    auto candidates2 = candidates(cset2, value);
    if (candidates2.size() < 2 || candidates2.size() > 3) return false;

    auto candidates3 = candidates(cset3, value);
    if (candidates3.size() < 2 || candidates3.size() > 3) return false;

    // Check that all three elimination sets contain at least 2 candidates
    auto eliminates1 = candidates(eset1, value);
    auto eliminates2 = candidates(eset2, value);
    auto eliminates3 = candidates(eset3, value);

    if (eliminates1.size() < 2 || eliminates2.size() < 2 || eliminates3.size() < 2) return false;

    // Check that at least one elimination set has more than 3 candidates (something to eliminate)
    if (eliminates1.size() <= 3 && eliminates2.size() <= 3 && eliminates3.size() <= 3) return false;

    // Verify the pattern: candidates from cset1 must be in the three esets
    std::set<const EliminationSet*> eset_ptrs = {&eset1, &eset2, &eset3};
    for (const auto &candidate : candidates1) {
        const EliminationSet* found_eset = nullptr;
        for (const auto* eset_ptr : eset_ptrs) {
            if (eset_ptr->contains(candidate)) {
                found_eset = eset_ptr;
                break;
            }
        }
        if (!found_eset) return false;
    }

    // Verify candidates from cset2 are in the three esets
    ...
    // Verify candidates from cset3 are in the three esets
    for (const auto &candidate : candidates3) {
        const EliminationSet* found_eset = nullptr;
        for (const auto* eset_ptr : eset_ptrs) {
            if (eset_ptr->contains(candidate)) {
                found_eset = eset_ptr;
...
bool Analyzer::find_swordfish(const Cell &cell, const Value &value, const CandidateSet &cset, const EliminationSet &eset,
    ...
    for (auto const &cset2 : csets) {
        ...
        for (auto const &cset3 : csets) {
            ...
            // Verify at least one elimination set has more than 3 candidates (something to eliminate)
            bool has_eliminations = false;
            for (const auto* eset_ptr : eset_total) {
                auto eset_candidates = candidates(*eset_ptr, value);
                if (eset_candidates.size() > 3) {
